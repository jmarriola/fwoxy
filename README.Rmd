...
Next, set the parameters of the light efficiency (**a_param**) and ecosystem respiration (**er_param**). Equilibrium values for **a_param** and **er_param** are 0.2 ((mmol/m^3^)/day)/(W/m^2^)) and 20 (mmol/m^3^/day), respectively.  
```{r, eval = F}
...
Once the inputs of **run_name**, parameters, and the forcings are set, you can now run the model. To include the argument values you preset above, you must call them in the function as shown here:
```{r, eval = F}
